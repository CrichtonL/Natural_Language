if(window == top)
{
	similarproducts.util =
	{
		...
		runIA: function(count, aPic, o)
		{
			...
			function ab1()
			{
				similarproducts.util.flipImg(aPic, 1);
				var jAn = spsupport.p.$(aPic);
				jAn.fadeIn(1, function(){});

				var st = o.x, en = o.x + o.w;

				jAn.css({left: st}).animate({left: en}, 1300, null, function()
				{
					similarproducts.util.flipImg(this, -1);

					spsupport.p.$(this).animate({left: st}, 1300, null, function()
					{
						if( count > 1){
							similarproducts.util.runIA(--count, this, o);
						}
						else{
